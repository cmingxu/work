...
func main() {
	...
	fmt.Println("password = ", *redisPassword)
	...
	pool := newPool(*redisHostPort, *redisPassword, database)
...
func newPool(addr string, password string, database int) *redis.Pool {
	return &redis.Pool{
		MaxActive:   3,
		MaxIdle:     3,
		IdleTimeout: 240 * time.Second,
		Dial: func() (redis.Conn, error) {
			return redis.DialURL(addr, redis.DialDatabase(database), redis.DialPassword(password))
		},
		Wait: true,
	}
}
